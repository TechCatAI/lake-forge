...
export default function SourceSystemPage() {
  const [data, setData] = useState<SourceSystem[]>([]);
  const [loading, setLoading] = useState(true);
  const firstCellRefs = useRef<Record<number, HTMLTableCellElement | null>>({});
  const [lastAdded, setLastAdded] = useState<number | null>(null);
  const [dirtyRows, setDirtyRows] = useState<Map<number, Partial<SourceSystem>>>(new Map());
  ...
  async function loadData() {
    try {
      setLoading(true);
      const rows = await fetchSourceSystems();
      setData(rows);
  ...
  function handleEdit<K extends keyof SourceSystem>(id: number, field: K, value: SourceSystem[K]) {
  ...
  async function saveChanges() {
    ...
    const results = await Promise.all(
      entries.map(([id, delta]) =>
        updateSourceSystem(id, delta).then(
          (row) => ({ id, row }),
          (err: APIError) => ({ id, err })
        )
      )
  ...
  function addRow(row: SourceSystem) {
    setData((d) => [row, ...d]);
    origData.current.set(row.id, row);
    setLastAdded(row.id);
  }

  async function handleDelete(id: number, row: SourceSystem, index: number) {
    setData((ds) => ds.filter((r) => r.id !== id));
    try {
      await deleteSourceSystem(id);
  ...
  const columns: ColumnDef<SourceSystem>[] = [
    ...
    },
    {
      ...
      cell: ({ row, getValue }) => (
        <EditableCell
          initialValue={getValue<string>()}
          onSave={(v) => handleEdit(row.original.id, "server", v)}
          className="text-left"
        />
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row, getValue }) => (
        <select
          ...
          onChange={(e) => handleEdit(row.original.id, "type", e.target.value as SourceSystem["type"])}
        >
        ...
        <thead className="sticky top-10 bg-background">
          {table.getHeaderGroups().map((hg) => (
            <tr key={hg.id}>
              ...
              <th className="px-3 py-2 bg-zinc-800 text-left text-gray-300" />
            </tr>
          ))}
        </thead>
        <tbody>
