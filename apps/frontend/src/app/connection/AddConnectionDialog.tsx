...
export default function AddConnectionDialog({ onCreate }: { onCreate(c: Connection): void }) {
  ...
  const valid = form.name.trim().length > 0 && !!form.conn_type;
  ...
  async function submit(e?: React.FormEvent) {
    ...
    try {
      ...
      if (form.driver_class) payload.driver_class = form.driver_class;
      if (form.endpoint_url) payload.endpoint_url = form.endpoint_url;
      if (form.secret_scope) payload.secret_scope = form.secret_scope;
